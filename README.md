...
- baseballr::scrape_statcast_savant() was used to collect pitch by pitch data from the 2019 season to the 2023 All-Star Break.
...
## There are 5 tabs available in this MLB Batting App.
...
- After filtering by player and season(s), there is a pitch type drop down menu followed by input sliders for velocity range, vert. & horiz. pitch breaks and spin rate to allow the user to get heat maps vs. a very specific pitch
  - adding input filters is fairly straight forward in RShiny, so X & Y release point sliders can easily be added, filter by count, etc.

- Each set of heat maps is separated by 'vs. LHP' and 'vs. RHP'

- The types of heat maps provided in this tab are:
  - **General pitch location**, **Whiffs** pitch location, **Takes** pitch location, **Swings** pitch location, **Hits** pitch location, **Barrels** pitch location, plus separate **Groundballs**, **Linedrives**, **Flyballs**, and **Popups** pitch location heat maps
  - adding different types of heat maps is made easy in my code using the ‘find_plots()’ function (view code).  Homerun pitch location heat maps could easily be added, Hard Hit pitch location heat map, etc.

- The higher the number is on the density key (to the right of each set of heat maps) the more concentrated the pitch locations are in one location
...
- After filtering by player, season(s), pitch type and pitcher handedness, there is a drop down menu to filter by stat type
  - The stat type filter options are: **Whiff%**, **Swing%**, **Take%**, **Average Exit Velocity**, **Average Launch Angle**, **Run Value**, **Hard Hit%**, **Sweet Spot%**, **Barrel%**, and **Called Strike + Whiff% (CsW%)**
...
* The zone is displayed from the catchers view
...
- After filtering by player, season(s), and pitch type, there is a drop down menu to filter by hit type
...
* The 2 tables show plate discipline stats and batted ball stats
  The spray charts are separated by 'vs. LHP' and 'vs. RHP'
* The graphic to the right of the spray charts is an interactive plotly map that shows a players hit profile according to launch angle and exit velo
...
- After filtering by player and season(s), there are additional filters that allow the user to filter by pitch count, pitch type and pitcher handedness 
...
* After filtering by player, there is a **pitch type filter** that will show how a player is trending vs. a certain pitch type, additionally there is a **date range filter** that can be used to find how a batters stats are trending over any period of time over the course of a season
